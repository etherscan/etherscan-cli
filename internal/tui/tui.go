...
var (
	...
	errColor  = lipgloss.Color(brand.ErrorHex)
	...
	hintKeySt = lipgloss.NewStyle().Bold(true).Foreground(accent)
)

// keyHint is one "[key] action" pair in a footer.
type keyHint struct{ key, action string }

// hintSep separates footer hints.
const hintSep = " · "

// renderHints formats footer hints as "[enter] open · [esc/q] quit". Bracketing
// and colouring the keys makes them scannable at a glance; previously they ran
// together with their labels in one dim sentence and were easy to miss.
func renderHints(hints ...keyHint) string {
	parts := make([]string, 0, len(hints))
	for _, h := range hints {
		parts = append(parts, footerSt.Render("[")+hintKeySt.Render(h.key)+footerSt.Render("] "+h.action))
	}
	return strings.Join(parts, footerSt.Render(hintSep))
}
...
func (m *model) openAPIKey() (tea.Model, tea.Cmd) {
	...
	ti.Prompt = "› "
	// Mask the key: a terminal screenshot or a shared screen would otherwise leak
	// it, and the value is validated against the API before saving so there is no
...
func (m model) viewAPIKey() string {
	...
	return join(m.header(), "", b.String(), m.footer(
		keyHint{"enter", "save & continue"},
		keyHint{"esc", "keep exploring"},
	))
}
...
func (m model) viewBrowse() string {
	...
	keys := []keyHint{{"↑/↓", "move"}, {"←/→", "pane"}, {"enter", "open"}}
	if m.cfg.SwitchChain != nil {
		keys = append(keys, keyHint{"c", "chain"})
	}
	keys = append(keys, keyHint{"esc/q", "quit"})
...
func (m model) viewResult() string {
	keys := []keyHint{{"↑/↓", "scroll"}}
	if m.current.Paginated {
		keys = append(keys, keyHint{"n/p", fmt.Sprintf("page (%d)", m.page)})
	}
	keys = append(keys, keyHint{"esc", "back"}, keyHint{"q", "quit"})
	if m.cfg.SwitchChain != nil {
		keys = append(keys, keyHint{"c", "chain"})
	}
