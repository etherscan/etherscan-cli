...
func TestRenderHintsFormat(t *testing.T) {
	got := stripANSIStr(renderHints(keyHint{"enter", "open"}, keyHint{"esc/q", "quit"}))
	if want := "[enter] open · [esc/q] quit"; got != want {
		t.Errorf("renderHints = %q; want %q", got, want)
	}
}

// The key must be styled separately from its label, not merely bracketed.
//
// Colour cannot be asserted directly: under `go test` lipgloss resolves to the
// Ascii profile and strips every escape, including bold. Swapping the key style
// for a marker proves renderHints routes the key — and only the key — through it.
func TestRenderHintsStylesKeyOnly(t *testing.T) {
	original := hintKeySt
	hintKeySt = hintKeySt.Transform(func(s string) string { return "<key>" + s + "</key>" })
	t.Cleanup(func() { hintKeySt = original })

	got := renderHints(keyHint{"enter", "open"})
	if want := "[<key>enter</key>] open"; got != want {
		t.Errorf("renderHints = %q; want %q — the key style must wrap the key alone", got, want)
	}
	if !original.GetBold() {
		t.Error("hint keys should be bold")
	}
	if fg, want := original.GetForeground(), lipgloss.Color(brand.AccentHex); fg != want {
		t.Errorf("hint key colour = %v; want the brand accent %v", fg, want)
	}
	if footerSt.GetForeground() == original.GetForeground() {
		t.Error("keys and labels must not share a colour, or the brackets are the only cue")
	}
}

// Footers must not wrap on a conventional 80-column terminal; a wrapped footer
// pushes the last hints onto a second line and reads as broken layout.
func TestFooterHintsFitEightyColumns(t *testing.T) {
	footers := map[string][]keyHint{
		"browse":  {{"↑/↓", "move"}, {"←/→", "pane"}, {"enter", "open"}, {"c", "chain"}, {"esc/q", "quit"}},
		"result":  {{"↑/↓", "scroll"}, {"n/p", "page (1)"}, {"esc", "back"}, {"q", "quit"}, {"c", "chain"}},
		"form":    {{"tab", "next field"}, {"enter", "submit"}, {"esc", "back"}},
		"chains":  {{"type", "filter"}, {"↑/↓", "move"}, {"enter", "switch"}, {"esc", "cancel"}},
		"api key": {{"enter", "save & continue"}, {"esc", "keep exploring"}},
	}
	for name, hints := range footers {
		if w := lipgloss.Width(renderHints(hints...)); w > 80 {
			t.Errorf("%s footer is %d cols; want <= 80", name, w)
		}
	}
}
...
func stripANSIStr(s string) string { return ansiPattern.ReplaceAllString(s, "") }

// The key must never be legible on screen: the explorer runs full-screen and a
// screenshot or shared session would otherwise expose it. The value is validated
// against the API before being saved, so it never needs to be read back.
func TestAPIKeyPromptMasksInput(t *testing.T) {
	cfg := Config{
		Endpoints:  []Endpoint{{Module: "stats", Action: "ethprice", Title: "ethprice"}},
		ChainName:  "ethereum",
		ChainID:    "1",
		KeyLabel:   "none",
		SaveAPIKey: func(context.Context, string) (string, error) { return "TES…KEY", nil },
	}
	m := newModel(context.Background(), cfg)
	m.Update(tea.WindowSizeMsg{Width: 100, Height: 30})
	m.focus = focusEndpoints
	...
	m.keyInput.SetValue("SuperSecretApiKey")
	view := m.View()
	if strings.Contains(view, "SuperSecretApiKey") {
		t.Errorf("API key prompt leaked the key:\n%s", view)
	}
	if !strings.Contains(view, strings.Repeat("*", len("SuperSecretApiKey"))) {
		t.Errorf("API key prompt should mask with asterisks:\n%s", view)
	}
}
